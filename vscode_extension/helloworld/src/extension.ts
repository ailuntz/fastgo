...
async function getConfiguration(): Promise<{baseUrl: string, defaultTemplateStyle: number, pathPrefix: string, containerPath: string} | null> {
	...
	if (!baseUrl) {
		...
		vscode.window.showInformationMessage(t('config.serverUrl.saved'));
	}
	...
	if (!pathPrefix) {
		...
		vscode.window.showInformationMessage(t('config.pathPrefix.saved'));
	}
...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.commands.registerCommand('extension.createShareLink', async (uri: vscode.Uri) => {
		...
		if (!config) {
			vscode.window.showWarningMessage(t('common.cancelled'));
			return;
		}

		// Check if path is valid for sharing
		if (!isPathValidForSharing(filePath, config.pathPrefix)) {
			vscode.window.showInformationMessage(t('share.pathNotValid'));
			return;
		}
		...
		vscode.window.showInformationMessage(t('share.success', { link: fullLink }));
		} catch (err) {
		vscode.window.showErrorMessage(t('common.createFailed', { error: (err as Error).message }));
		}
	})
	);
	...
	context.subscriptions.push(
		...
		const fullLink = `${config.baseUrl}/${res.link}`;  // 拼接完整的链接
		vscode.window.showInformationMessage(t('upload.success', { link: fullLink }));
		} catch (err) {
		vscode.window.showErrorMessage(t('common.createFailed', { error: (err as Error).message }));
		}
