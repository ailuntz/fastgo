...
class I18n {
    ...
    private loadLocale(locale: string): void {
        try {
            // Try both src (development) and out (production) paths
            let localePath = path.join(this.extensionPath, 'out', 'i18n', 'locales', `${locale}.json`);
            
            if (!fs.existsSync(localePath)) {
                localePath = path.join(this.extensionPath, 'src', 'i18n', 'locales', `${locale}.json`);
            }
            ...
            if (fs.existsSync(localePath)) {
                ...
                let fallbackPath = path.join(this.extensionPath, 'out', 'i18n', 'locales', 'en.json');
                
                if (!fs.existsSync(fallbackPath)) {
                    fallbackPath = path.join(this.extensionPath, 'src', 'i18n', 'locales', 'en.json');
                }
